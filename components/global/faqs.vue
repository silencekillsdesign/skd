  ...
  <v-container justify-center mb-5>
    ...
    <v-container>
      <v-layout row wrap mb-5 py-5 justify-center>
        <h2
          class="display-3 text-uppercase font-weight-bold indigo--text text--accent-3" data-aos-delay="200">Common Questions</h2>
      </v-layout>
    </v-container>
    ...
    <v-layout row wrap justify-center>
      <v-flex md8 xs12 md mb-3>
        <v-expansion-panel popout focusable dark>
          <!-- <v-expansion-panel-content lazy v-for="(item,i) in 5" :key="i"> -->
          ...
          >
            <template v-slot:header>
              <!-- <div>Item</div> -->
              <h4 class="title">{{ faq.faqQ}}</h4>
            </template>
            <v-card class="indigo accent-4 pa-2">
              <v-card-text class="subheading">{{ faq.faqA }}</v-card-text>
              <v-btn
        ...
        <v-expansion-panels>
          <v-expansion-panel v-for="(faq,i) in faqs" :key="i">
            <v-expansion-panel-header class="title">{{ faq.faqQ}}</v-expansion-panel-header>
            <v-expansion-panel-content>
              <p>{{ faq.faqA }}</p>
              <v-btn
                v-if="faqs.faqButtonLink !== null"
  ...
  data() {
    return {
      ...
      faqs: [
        ...
        {
          faqQ: 'How much does a website cost?',
          faqA:
            'There are too many factors in creating a website to give a generic number here. However, a static 4-page website (home, about, services, contact) typically runs about $1200 - $1500. ',
          faqButtonText: 'Get a Website Quote',
          faqButtonLink: 'contact',
          faqCategories: ['web'],
          faqId: 3,
          faqIcon: 'mdi-help'
        ...
        {
          faqQ: 'Do you work with WordPress sites?',
          faqA:
